...
class MultiWayTransformerLayer(nn.Module):
    ...
    def forward(self, x, modality_mask, attention_mask=None):
        ...
        for batch_idx in range(batch_size):
            ...
            if vision_mask.any():
                vision_tokens = x[batch_idx, vision_mask]
                if vision_tokens.numel() > 0:
                    vision_out = self.vision_expert(vision_tokens)
                    vision_out = vision_out.to(expert_output.dtype)  # ép dtype
                    expert_output[batch_idx, vision_mask] = vision_out
            
            # Language tokens -> language expert
            language_mask = (modality_mask[batch_idx] == 1)
            if language_mask.any():
                language_tokens = x[batch_idx, language_mask]
                if language_tokens.numel() > 0:
                    lang_out = self.language_expert(language_tokens)
                    lang_out = lang_out.to(expert_output.dtype)  # ép dtype
                    expert_output[batch_idx, language_mask] = lang_out
            
            # Fusion tokens -> vision-language expert (nếu có)
            if self.is_fusion_layer:
                fusion_mask = (modality_mask[batch_idx] == 2)
                if fusion_mask.any():
                    fusion_tokens = x[batch_idx, fusion_mask]
                    if fusion_tokens.numel() > 0:
                        fusion_out = self.vision_language_expert(fusion_tokens)
                        fusion_out = fusion_out.to(expert_output.dtype)  # ép dtype
                        expert_output[batch_idx, fusion_mask] = fusion_out
...
class VQKDVisualTokenizer(nn.Module):
    ...
    """
    def __init__(self, vocab_size=8192, embed_dim=1024, teacher_dim=1024):  # Thay dims=1024 cho large
        super().__init__()
        ...
        # Teacher model (sử dụng pre-trained BEiT-large để match ViT-large)
        self.teacher_model = ViTModel.from_pretrained("microsoft/beit-large-patch16-224-pt22k-ft22k")  # Thay large variant, pre-train 22k classes
        for param in self.teacher_model.parameters():
            param.requires_grad = False  # Freeze teacher
        
        # Projection layer nếu teacher_dim != embed_dim
        if teacher_dim != embed_dim:
            self.proj_layer = nn.Linear(teacher_dim, embed_dim)
        else:
        ...
        self.decoder = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(
                d_model=embed_dim,
                nhead=16,  # Tăng heads=16 cho large (match ViT-large)
    ...
    def forward(self, images, return_loss=True):
        ...
        with torch.no_grad():
            ...
            teacher_features = teacher_outputs.last_hidden_state  # [batch, patches, 1024] cho large
        
        # 2. Encode patches -> h_i (sử dụng teacher features làm encoder features)
        patch_features = teacher_features
        if self.proj_layer is not None:
            patch_features = self.proj_layer(patch_features)
...
class ImprovedVietnameseVQAModel(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        vision_dim = self.vision_model.config.hidden_size  # 1024 cho large
        
        # Text encoder (PhoBERT-large)
        self.text_model = AutoModel.from_pretrained(model_config['text_model'])
        text_dim = self.text_model.config.hidden_size  # 1024
        
        # Text decoder (BARTPho-syllable)
        ...
        # VQ-KD Visual Tokenizer (cập nhật dims=1024)
        ...
        num_multiway_layers = model_config.get('num_multiway_layers', 8)  # Tăng cho large (config update)
        
        assert hidden_dim == 1024 and vision_dim == 1024 and text_dim == 1024, "All dims must be 1024 for large models"
        
        self.fusion_layer = MultiwayFusionLayer(
            vision_dim, text_dim, hidden_dim, num_multiway_layers, dropout_rate
        )
        
        # Output projection (giữ, dims match)
        decoder_dim = self.text_decoder.config.d_model  # 1024 cho syllable
        assert decoder_dim == 1024, "Decoder dim must be 1024 for syllable large"
        ...
        # Pool target length
        self.pool_target_length = 64  # Tăng từ 32
        self.pool_queries = nn.Parameter(torch.randn(self.pool_target_length, decoder_dim))
        self.pool_attn = nn.MultiheadAttention(decoder_dim, num_heads=16, batch_first=True)  # Tăng heads=16 cho large
    ...
    def apply_masking(self, tokens, mask_ratio=0.15, is_vision=False, mask_token_id=None):
        """
        Apply masking strategy.
        - tokens: tensor [batch, seq_len, ...] (or [batch, seq_len] for token ids)
        - is_vision: nếu True -> mask patch features (float) (zero out)
        - mask_token_id: token id dùng để thay thế khi masking text tokens (nên truyền question tokenizer.mask_token_id)
        """
        ...
        if is_vision:
            ...
            # zero out patch features where mask True
            masked_tokens[mask] = 0.0
            return masked_tokens, mask
        else:
            # tokens are token ids (long)
            mask = torch.rand(batch_size, seq_len, device=tokens.device) < mask_ratio
            masked_tokens = tokens.clone()

            # determine mask_token_id fallback logic
            if mask_token_id is None:
                # try config provided question mask id
                mask_token_id = getattr(self, 'question_mask_token_id', None)
                if mask_token_id is None:
                    mask_token_id = self.config.get('question_mask_token_id', None) if hasattr(self, 'config') else None

            if mask_token_id is None:
                # final fallback: use decoder's mask id if available, else pad token id, else 0
                if hasattr(self, 'decoder_tokenizer') and getattr(self.decoder_tokenizer, 'mask_token_id', None) is not None:
                    mask_token_id = self.decoder_tokenizer.mask_token_id
                else:
                    # Use pad token id as safe fallback (better than random large id)
                    mask_token_id = getattr(self.text_model.config, 'pad_token_id', 0)

                # Warn user in logs (only once)
                if getattr(self, '_mask_fallback_warned', False) is False:
                    print("Warning: question_mask_token_id not provided; using fallback mask token id:", mask_token_id)
                    self._mask_fallback_warned = True

            # apply mask (works for long tensors)
    ...
    def forward(self, pixel_values, question_input_ids, question_attention_mask, 
        ...
        if answer_input_ids is not None:  # Training mode
            ...
            generated_ids = self.text_decoder.generate(
                encoder_outputs=encoder_outputs,
                max_length=64,  # Tăng từ 32 cho syllable
                min_length=1,  # Điều chỉnh min nếu answers short
                num_beams=4,
                early_stopping=True,
    ...
    def create_pooled_representation(self, encoder_states, attention_mask, target_length=None):
        ...
        # Add positional embeddings (match query length)
        target_length = min(target_length, queries.size(1))
        pos_ids = torch.arange(target_length, device=queries.device)
        pos_emb = self.pool_pos_emb(pos_ids).unsqueeze(0)
        queries = queries[:, :target_length, :] + pos_emb
